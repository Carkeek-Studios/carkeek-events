<?php
/**
 * Render callback for carkeek-events/event-details.
 *
 * Displays the date/time, location, and organizer for a Carkeek Event.
 *
...
// Prime the object cache for the event and its linked location and organizer.
...
$wrapper_attrs = get_block_wrapper_attributes( array( 'class' => 'carkeek-event-details' ) );
...
echo '<div ' . $wrapper_attrs . '><div class="carkeek-event-details-inner">';
foreach ( $parts as $part ) {
	if ( !empty( $part ) ) {
		echo '<div class="carkeek-event-detail">' . wp_kses_post( $part ) . '</div>';
	}
}
echo '</div></div>';
